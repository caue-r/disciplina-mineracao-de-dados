...
def main():
    ...
    for r in registros[:10]:
        print(f"{r['data']:<12} {r['dia_semana']:<15} {r['hora']:<6} {r['altura_m']:<10} {r['tipo_mare']}")
 
    meses = sorted({r["data"][:7] for r in registros})
    print(f"\nMeses  : {', '.join(meses)}")
    print(f"Eventos: {len(registros)}")
    print(f"Dias   : {len({r['data'] for r in registros})}")
